...
@dataclass
class DiffuEraserPipelineStageOneOutput(BaseOutput):
    frames: Union[torch.Tensor, np.ndarray]

class StableDiffusionDiffuEraserPipelineStageOne(
    DiffusionPipeline,
    ...
    r"""
    Pipeline for video inpainting using Video Diffusion Model with BrushNet guidance. Stage1.
    ...
    def prepare_latents(self, batch_size, num_channels_latents, num_frames, height, width, dtype, device, generator, latents=None):
        ...
        if latents is None:
            ...
            noise = randn_tensor(shape, generator=generator, device=device, dtype=dtype)
            noise = repeat(noise, "1 c h w -> bt c h w", bt=batch_size*num_frames)
        else:
        ...
        **kwargs,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                ## update
                if self.do_classifier_free_guidance:
                    negative_prompt_embeds_t, prompt_embeds_t = prompt_embeds.chunk(2)
                    negative_prompt_embeds_r = rearrange(repeat(negative_prompt_embeds_t, "b c d -> b t c d", t=num_frames), 'b t c d -> (b t) c d')
                    prompt_embeds_r = rearrange(repeat(prompt_embeds_t, "b c d -> b t c d", t=num_frames), 'b t c d -> (b t) c d')
                    prompt_embeds_unet = torch.cat([negative_prompt_embeds_r, prompt_embeds_r])
                else:
                    prompt_embeds_unet = rearrange(repeat(prompt_embeds, "b c d -> b t c d", t=num_frames), 'b t c d -> (b t) c d')
                ...
                noise_pred = self.unet(
                    latent_model_input,
                    t,
                    encoder_hidden_states=prompt_embeds_unet,
        ...
        if  output_type == "latent":
            image = latents
            has_nsfw_concept = None
            return DiffuEraserPipelineStageOneOutput(frames=image, nsfw_content_detected=has_nsfw_concept)
        ...
        return DiffuEraserPipelineStageOneOutput(frames=video)
